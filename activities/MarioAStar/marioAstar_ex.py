...
def melhor_filho(tree):
    ...
    melhores = [melhor_filho(v) for k, v in tree.filhos.items()]
    #f_none =  [v for k, v in tree.filhos.items() if melhor_filho(v) is None]
    f_none = [f for f in melhores if f is None]

    # 4) Se todos os filhos resultarem em terminal, marca tree como terminal e retorna None
    #f_term = sum(1 for k, f in tree.filhos.items() if f.eh_terminal)
    #if f_term and f_term == len(f_none):
    if len(f_none) == len(tree.filhos):
        tree.eh_terminal = True
        return None
    
    # 5) Caso contrário retorna aquele com o menor f
    #min_f = min([v for k, v in tree.filhos.items()], key=lambda k: k.g + k.h)
    min_f = min(melhores, key=lambda f: f[1])
    #return (min_f, min_f.g + min_f.h)
    return min_f
        ...
        while raiz.pai is not None:
            ...
            for k, v in moves.items():
                if raiz.filhos[k] == neto:
                    acoes.append(v)
                    break
            #for k, v in raiz.filhos.items():
            #    if v == neto:
            #        acoes.append(moves[k])
            #        break
